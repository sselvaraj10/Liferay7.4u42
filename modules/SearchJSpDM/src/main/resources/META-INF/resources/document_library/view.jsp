...
<%@ page import="com.liferay.frontend.taglib.clay.servlet.taglib.util.DropdownItemListBuilder" %><%--
/**
...
<%
	...
	String searchId = 	dlSearchContainer.getId(request,liferayPortletResponse.getNamespace());
	List<DropdownItem> dropdownItemList = dlAdminManagementToolbarDisplayContext.getFilterDropdownItems();
	if(dropdownItemList!=null) {
		List<DropdownItem> mydropdownitem = new DropdownItemListBuilder().addGroup(
				dropdownGroupItem -> {
							...
							new DropdownItemList() {
								{
									...
									add(
											dropdownItem -> {
												dropdownItem.setHref("http://localhost:8080/group/guest/~/control_panel/manage?p_p_id=com_liferay_document_library_web_portlet_DLAdminPortlet&p_v_l_s_g_id=20121&_com_liferay_document_library_web_portlet_DLAdminPortlet_fileEntryTypeId=0");
		...
		dropdownItemList.addAll(mydropdownitem);
	}
	...
	<c:otherwise>
		...
		<clay:management-toolbar
			...
			%>'
				...
				searchContainerId="<%= searchId %>"
				filterDropdownItems="<%= dropdownItemList%>"/>
